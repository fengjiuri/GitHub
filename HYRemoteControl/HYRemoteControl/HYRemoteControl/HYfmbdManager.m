...
@interface HYfmbdManager (){
    FMDatabase *db;
}

@end
@implementation HYfmbdManager

+(HYfmbdManager *)shareManager{
    static HYfmbdManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[HYfmbdManager alloc]init];
    });
    return manager;
}
...
-(BOOL)checkData:(NSString *)lac cell:(NSString *)cell{
    ...
    NSString *checkSql = [NSString stringWithFormat:
                           @"select * FROM AddressInfo WHERE LAC = '%@' AND CELL = '%@'",lac,cell];
...
-(NSMutableArray *)searchData{
    ...
    int index = 0;
    while (rs.next) {
        HYAdressInfo *tmp = [[HYAdressInfo alloc]init];
        index++;
